...
import { removeFromCart, setAmount } from '../../../../entities/cart/model/actions';
...
const CartProduct: FC<ICartProduct> = memo(({ title, price, images, description, amount, id }) => {
  ...
  return (
    <>
      ...
      <div >
        <Field>
          <Input onChange={(e: ChangeEvent<HTMLInputElement>) => setLocalAmount(Number(e.target.value))}
            value={localAmount}
            type="number"
            min="1"
            max="10" />
      ...
      <div >
        <Button onClick={() => removeProduct(id)} size={SIZE.SMALL} variant={COLORS.DANGER}>
          Remove
        </Button>
      </div>
    </>
